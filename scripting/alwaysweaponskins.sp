...
{
	Handle config = LoadGameConfigFile("alwaysweaponskins.games");
	if(config == null)
	{
	...
	{
		CloseHandle(config);
		LogError("Unable to find offset 'GiveNamedItem' in game data 'alwaysweaponskins.games'");
		return false;
	}
...
public MRESReturn OnGiveNamedItemPre(int client, Handle hReturn, Handle hParams)
{
	...
	{
		if (s_bDebugMessages)
			PrintToChat(client, "[AWS] -> Plugin is disabled");
		return MRES_Ignored;
	}
	...
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] OnGiveNamedItemPre(int client, char[] classname='%s')", classname);

	int iDefIndex = GetItemDefinitionByClassname(classname);

	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] OnGiveNamedItemPre(int client, char[] classname='%s' iDefIndex: '%d')", classname, iDefIndex);

	//if(iDefIndex == 64 || iDefIndex == 63 || iDefIndex == 61 || iDefIndex == 3 || iDefIndex == 32 || iDefIndex == 60 || iDefIndex == 16 || iDefIndex == 30)
	...
	if (IsItemDefinitionKnife(iDefIndex))
		return MRES_Ignored;

	int weaponteam = GetWeaponTeamByItemDefinition(iDefIndex);

	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] -> Item definition team: %d", weaponteam);
	...
	{
		if (s_bDebugMessages)
			PrintToChat(client, "[AWS] -> Skipped, player on correct team");
		return MRES_Ignored;
	}
	...
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] -> Player.m_iTeamNum set to %d", weaponteam);
	return MRES_Ignored;
}
...
{
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] OnGiveNamedItemPost(int client, char[] classname)");
	...
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] -> Player.m_iTeamNum set to %d", s_OriginalClientTeam);
	s_HookInUse = false;
	return MRES_Ignored;
}
...
public Action OnPostWeaponEquip(int client, int weapon)
{
	...
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] OnPostWeaponEquip(weapon=%d)", weapon);
	...
	{
		if (s_bDebugMessages)
			PrintToChat(client, "[AWS] -> Skip Map Weapons is enabled");
		return Plugin_Continue;
	}
	...
	{
		if (s_bDebugMessages)
			PrintToChat(client, "[AWS] -> Skipped: IsMapWeapon(weapon=%d)==false", weapon);
		return Plugin_Continue;
	}
	...
	switch (itemdefinition)
	{
		...
		{
			if (s_bDebugMessages)
				PrintToChat(client, "[SM] -> Index 60: Classname reset to: weapon_m4a1_silencer from: %s", classname);
			classname = "weapon_m4a1_silencer";
		}
		case 61:
		{
			if (s_bDebugMessages)
				PrintToChat(client, "[SM] -> Index 61: Classname reset to: weapon_usp_silencer from: %s", classname);
			classname = "weapon_usp_silencer";
		}
		case 63:
		{
			if (s_bDebugMessages)
				PrintToChat(client, "[SM] -> Index 63: Classname reset to: weapon_cz75a from: %s", classname);
			classname = "weapon_cz75a";
		}
		case 64:
		{
			if (s_bDebugMessages)
				PrintToChat(client, "[SM] -> Index 64: Classname reset to: weapon_revolver from: %s", classname);
	...
	{
		if (s_bDebugMessages)
			PrintToChat(client, "[AWS] -> Skipped: m_hPrevOwner == %d", m_hPrevOwner);
		return Plugin_Continue;
	}
	...
	if (s_bSkipNamedWeapons)
	{
		...
		{
			if (s_bDebugMessages)
				PrintToChat(client, "[AWS] -> Skipped: m_iName == %s", entname);
			return Plugin_Continue;
		}
	}
	...
	// Debug logging
	if (s_bDebugMessages)
		PrintToChat(client, "[AWS] Respawning %s (defindex=%d)", classname, itemdefinition);
