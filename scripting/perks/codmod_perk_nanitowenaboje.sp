...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Nanitowe Naboje",
    author = "th7nder",
...
char szClassName[] = {"Nanitowe Naboje"};
char szDesc[] = {"Zadajesz 5(+0.3/1INT) obrażeń więcej."};
int g_iPerkId;
...
public void CodMod_OnPlayerDamagedPerk(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        fDamage += 5.0 + float(CodMod_GetWholeStat(iAttacker, INT) / 3);
    }
}
