...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - SSG08 Snajper",
    author = "th7nder",
...
char szClassName[] = {"SSG08 Snajper"};
char szDesc[] = {"Na początku rundy dostajesz SSG08 \n Masz 1/2 z SSG08"};
int g_iPerkId;
...
const WeaponID g_iGiveWeapon = WEAPON_SSG08;
char g_szGiveWeapon[] = "weapon_ssg08";
...
public void CodMod_OnPlayerDamagedPerk(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        if(iWeaponID == g_iGiveWeapon && GetRandomInt(1, 100) >= 50){
            fDamage *= 300.0;
        }
    }
}
