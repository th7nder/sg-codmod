...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Zestaw Paranoika",
    author = "th7nder",
...
char szClassName[] = {"Zestaw Paranoika"};
char szDesc[] = {"Na początku rundy dostajesz XM014 \n 1/10 na natychmiastowe zabójstwo."};
int g_iPerkId;
...
const WeaponID g_iGiveWeapon = WEAPON_XM1014;
char g_szGiveWeapon[] = "weapon_xm1014";
...
public void CodMod_OnPlayerDamagedPerk(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        if(iWeaponID == g_iGiveWeapon && GetRandomInt(1, 25) == 1){
            fDamage *= 300.0;
        }
    }
}
