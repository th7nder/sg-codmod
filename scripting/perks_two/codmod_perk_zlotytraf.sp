...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Złoty Traf",
    author = "th7nder",
...
char szClassName[] = {"Złoty Traf"};
char szDesc[] = {"Masz 1/8 szans na wyrzucenie broni przeciwnika"};
int g_iPerkId;
...
public void CodMod_OnPlayerDamagedPerk(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        if(GetRandomInt(1, 8) == 1){
            ...
            if(iEntity != -1){
                CS_DropWeapon(iVictim, iEntity, true);
            } else {
                iEntity = GetPlayerWeaponSlot(iVictim, 1);
                if(iEntity != -1){
                    CS_DropWeapon(iVictim, iEntity, true);
                }
            }
