...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Karabin Kaprala",
    author = "th7nder",
...
char szClassName[] = {"Karabin Kaprala"};
char szDesc[] = {"Na początku rundy dostajesz M4A4 \n 1/15 na natychmiastowe zabójstwo."};
int g_iPerkId;
...
public void CodMod_OnPlayerDamaged(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        if(iWeaponID == g_iGiveWeapon && GetRandomInt(1, 15) == 1){
            fDamage *= 300.0;
        }
    }
}
