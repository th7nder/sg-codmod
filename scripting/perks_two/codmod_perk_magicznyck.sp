...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Magiczny cK",
    author = "th7nder",
...
char szClassName[] = {"Magiczny cK"};
char szDesc[] = {"Na początku rundy dostajesz MAG-7 \n 1/12 na natychmiastowe zabójstwo."};
int g_iPerkId;
...
const WeaponID g_iGiveWeapon = WEAPON_MAG7;
char g_szGiveWeapon[] = "weapon_mag7";
...
public void CodMod_OnPlayerDamagedPerk(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iAttacker]){
        if(iWeaponID == g_iGiveWeapon && GetRandomInt(1, 20) == 1){
            fDamage *= 300.0;
        }
    }
}
