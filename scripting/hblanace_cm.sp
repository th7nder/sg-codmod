...
balanceBySwap(leadingTeam, bool force, bool slay) {
    ...
    for (new i=0; i < getTeamTableSize(leadingTeam); i++)
    {
        ...
        for (new j=0; j < getTeamTableSize(getOpposingTeamOf(leadingTeam)); j++)
        {
            ...
            {
                int ifScore = GetArrayCell(getTeamTable(getOpposingTeamOf(leadingTeam)), j, 0);
    ...
    if ((bestGuessLT < 0) || (bestGuessIT < 0))
    {
        ...
        else if (force)
        {
            ...
            int ifScore = GetArrayCell(getTeamTable(getOpposingTeamOf(leadingTeam)), bestGuessIT, 0);
    ...
    int clientIT = GetArrayCell(getTeamTable(getOpposingTeamOf(leadingTeam)), bestGuessIT, 1);
...
getPlayerScore(client)
{
    ...
    if ( isValidPlayer(client) )
    {
        int kills = CodMod_GetLevel(client);//GetClientFrags(client);
