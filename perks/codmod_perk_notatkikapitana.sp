...
public Plugin myinfo = {
    name = "Call of Duty Mod - Perk - Notatki Kapitana",
    author = "th7nder",
...
char szClassName[] = {"Notatki Kapitana"};
char szDesc[] = {"Masz 20%% szans na odbicie obrażeń."};
int g_iPerkId;
...
public void CodMod_OnPlayerDamaged(int iAttacker, int iVictim, float &fDamage, WeaponID iWeaponID, int iDamageType){
    if(g_bHasItem[iVictim] && GetRandomInt(1, 100) >= 80){
        PrintToChat(iVictim, "%s Damage odbity!", PREFIX_SKILL);
        //odMod_DealDamage(iVictim, iAttacker, fDamage, TH7_DMG_REFLECT);
        fDamage = 0.0;
    }
}
